...
**Cache-busting de la whitelist**: GitHub Pages sirve los assets estáticos con `Cache-Control: max-age=600` (10 minutos) — sin nada más, alguien que ya tenía la página cargada podía seguir viendo la whitelist vieja hasta 10 minutos después de un `git push` que agregara/sacara un email (ya pasó una vez). Por eso el `import` de `allowlist.js` en `auth.js` lleva un query string con el timestamp del build (`?v={{ site.time | date: '%s' }}`) — al cambiar esa URL en cada build, el navegador la trata como un recurso nuevo y la pide de cero, sin esperar a que expire el caché de la versión anterior. Esto requiere que `assets/js/auth.js` tenga front matter vacío (`---\n---`) al principio del archivo para que Jekyll corra Liquid sobre él — **gotcha si se toca**: sin ese front matter, Jekyll copia el archivo tal cual sin interpretar `{{ site.time | ... }}`, y el import queda roto con el texto literal de la plantilla Liquid en vez de un número. Esto solo evita el caché la próxima vez que alguien *recargue* la página — no empuja el cambio a una pestaña ya abierta.
...
**Gotcha si se toca esto**: el SDK de Firebase se carga vía CDN de Google (`https://www.gstatic.com/firebasejs/12.18.0/...`) como módulos ES (`import` en `auth.js`), sin bundler ni build step — mismo criterio de "sin build step" que el resto del sitio, aunque Firebase recomienda un bundler para producción. Si se actualiza la versión, hay que cambiarla en los tres `import` de `auth.js` a la vez (`firebase-app.js`, `firebase-auth.js`). El import de `allowlist.js` es relativo al propio `auth.js`, no pasa por `relurl.html` — los imports de módulos ES se resuelven relativos a la URL del módulo que importa, no a la página, así que funciona igual sin importar la profundidad de la URL actual (el query string de cache-busting, ver más abajo, no cambia esto).
